...
/** GET — list attachments on this post. */
export async function GET(_req: Request, { params }: Params) {
  return withErrorLog("posts.attachments.list", async () => {
...
/** POST — link an existing Attachment row to this post. */
export async function POST(req: Request, { params }: Params) {
  return withErrorLog("posts.attachments.link", async () => {
    const auth = await requireAuth();
    if (auth.error) return auth.error;
    const { user } = auth;
    ...
    await prisma.$transaction(async (tx) => {
      await tx.postAttachment.upsert({
        where: { postId_attachmentId: { postId: id, attachmentId } },
        create: { postId: id, attachmentId },
        update: {},
      });
      await writeAudit(tx, {
        actorUserId: user.id,
        action: "post_attachment.linked",
        entityType: "PostAttachment",
        entityId: attachmentId,
        diff: { postId: { old: null, new: id } },
      });
    });
