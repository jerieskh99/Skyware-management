...
function blankLine(): LineRow {
  return { description: "", amountAgorot: 0 };
}

export function CreateDraftDialog({ open, onOpenChange, clients }: Props) {
  ...
  const [type, setType] = useState<ReceiptDocumentType>("tax_invoice_receipt");
  const [clientId, setClientId] = useState<string>(clients[0]?.id ?? "");
  const [issueDate, setIssueDate] = useState<string>(todayLocalIso());
  ...
  const [vatRateBp, setVatRateBp] = useState<number>(1800);
  const [notes, setNotes] = useState<string>("");
  const [lines, setLines] = useState<LineRow[]>([blankLine()]);

  // Live computation, server re-computes authoritative values.
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!clientId) {
      setError(t("receipts.draftForm.clientPlaceholder"));
      return;
    }
    ...
    startTransition(async () => {
      ...
      toast.push({
        tone: "success",
        title: t("receipts.draftForm.createSuccess"),
      });
      onOpenChange(false);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{t("receipts.draftForm.createTitle")}</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={submit} className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-medium">{t("receipts.draftForm.client")}</label>
              <select
                value={clientId}
              ...
              >
                {clients.length === 0 && (
                  <option value="">{t("receipts.draftForm.clientPlaceholder")}</option>
                )}
                {clients.map((c) => (
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <p className="text-[11px] text-muted-foreground">
                {t("receipts.draftForm.amountHint")}
              </p>
            </div>
            <div className="space-y-1.5">
              ...
              />
              <p className="text-[11px] text-muted-foreground">
                {t("receipts.draftForm.vatRateHint")}
              </p>
            </div>
          </div>
